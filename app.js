...
function editBreakTimes() {
  ...
  openDialog(
    "Break Times",
    [
      { label: "Break 1", name: "break1", value: breaks[0].time, placeholder: "Example: 10:30 PM" },
      { label: "Break 1 Taken", name: "break1Taken", type: "checkbox", checked: breaks[0].taken },
      { label: "Break 2", name: "break2", value: breaks[1].time, placeholder: "Example: 2:30 AM" },
      { label: "Break 2 Taken", name: "break2Taken", type: "checkbox", checked: breaks[1].taken }
    ],
    (values) => {
      data.breakTimes = [
        { time: values.break1.trim(), taken: values.break1Taken === "on" },
        { time: values.break2.trim(), taken: values.break2Taken === "on" }
      ];
      saveData();
      render();
    }
...
function handleBreakTimesClick() {
  ...
  if (dueIndex >= 0) {
    data.breakTimes[dueIndex].taken = true;
    saveData();
    render();
    return;
  }
...
function getWeatherErrorMessage(error) {
  if (String(error?.message || "").includes("Failed to fetch")) {
    return "Weather lookup could not reach the live weather service. Open the GitHub Pages link, not the local file, and confirm the network allows weather.gov.";
  }
...
async function geocodeDaneCountyAddress(address) {
  ...
  if (!/Dane/i.test(county)) throw new Error("That address did not match Dane County.");
