...
const CAD_COPY_SHORTCUTS = {
  "911A": "911 ABANDONED CALL",
  "911U": "CONFIRMED ACCIDENTAL 911 CALL // CALLER CONFIRMED NO NEEDS FOR ANY EMERGENCY SERVICE // CALLER EXHIBITED NO INDICATORS OF DISTRESS",
  "911U Pocket": "CONFIRMED ACCIDENTAL 911 CALL (POCKET DIAL) // CALLER CONFIRMED NO NEEDS FOR ANY EMERGENCY SERVICE // CALLER EXHIBITED NO INDICATORS OF DISTRESS",
  "911O": "911 OPEN LINE",
  "Patient Deteriorating": "PATIENT DETERIORATING/CHANGE IN CONDITION: "
};
...
const els = {
  ...
  stationMovingInfo: document.querySelector("#stationMovingInfo"),
  ...
  breakStation: document.querySelector("#breakStation"),
  ...
  copy911AButton: document.querySelector("#copy911AButton"),
  copy911UButton: document.querySelector("#copy911UButton"),
  copy911UPocketButton: document.querySelector("#copy911UPocketButton"),
  copy911OButton: document.querySelector("#copy911OButton"),
  copyPatientDeterioratingButton: document.querySelector("#copyPatientDeterioratingButton"),
  ...
  weatherClearButton: document.querySelector("#weatherClearButton"),
  ...
  dispatcherAssignedButton: document.querySelector("#dispatcherAssignedButton"),
...
function prepareData(nextData) {
  ...
  nextData.stationMoving = Boolean(nextData.stationMoving);
  nextData.stationMovingTo = nextData.stationMovingTo || "";
  nextData.stationMovingTime = nextData.stationMovingTime || "";
...
function normalizeBreakTimes(breakTimes) {
  const normalized = Array.isArray(breakTimes) ? breakTimes.slice(0, 3) : [];
  while (normalized.length < 3) normalized.push({ time: "", taken: false });
...
function render() {
  ...
  els.stationButton.classList.toggle("is-station-moving", Boolean(data.stationMoving));
  const movingLabel = data.stationMoving && data.stationMovingTo
    ? `Moving To: ${data.stationMovingTo}${data.stationMovingTime ? ` @ ${data.stationMovingTime}` : ""}`
    : "";
  els.stationMovingInfo.textContent = movingLabel;
  els.stationMovingInfo.hidden = !movingLabel;
...
function renderBreakTimes() {
  ...
  const activeBreaks = breaks.filter((item) => item.time);
  const allActiveBreaksTaken = activeBreaks.length > 0 && activeBreaks.every((item) => item.taken);
  els.breakTimesButton.classList.toggle("is-breaks-complete", allActiveBreaksTaken);
  els.breakTimesButton.classList.toggle("has-break-taken", activeBreaks.some((item) => item.taken) && !allActiveBreaksTaken);
...
function renderPrcfsAlert() {
  ...
  if (!activeStatuses.length) {
    ...
    document.body.classList.remove("is-prcfs-mode");
    return;
  }
  ...
  document.body.classList.add("is-prcfs-mode");
...
function renderChannelUseAlert() {
  ...
  const chips = data.channelUses
    .map((item) => `<span>${item.dispatcherAssigned ? '<b class="dispatcher-badge">DSP</b>' : ""}${escapeHtml(formatChannelUseLabel(item))}</span>`)
    .join("");
  ...
  data.channelUses.forEach((item) => {
    els.channelUseAlertList.append(statusBannerRow(formatChannelUseLabel(item), () => deleteChannelUse(item.id), item.dispatcherAssigned));
  });
}

function statusBannerRow(text, onClear, showDispatcher = false) {
  ...
  if (showDispatcher) {
    ...
    badge.className = "dispatcher-badge";
...
function formatChannelUseLabel(item) {
  return item.callId ? `${item.channel} #${item.callId}` : item.channel;
}
...
function renderChannelUses() {
  ...
  data.channelUses.forEach((item) => {
    ...
    card.innerHTML = `
      <div>
        <div class="status-channel">${item.dispatcherAssigned ? '<b class="dispatcher-badge">DSP</b>' : ""}${escapeHtml(item.channel)}</div>
        ${item.callId ? `<div class="call-id">#${escapeHtml(item.callId)}</div>` : ""}
      </div>
    `;
...
function editStationAssignment() {
  openDialog(
    "Station Assignment",
    [
      { label: "Current Station", name: "stationAssignment", value: data.stationAssignment === "Set Station" ? "" : data.stationAssignment, required: true },
      { label: "Moving Later", name: "stationMoving", type: "checkbox", checked: data.stationMoving },
      { label: "Moving To", name: "stationMovingTo", value: data.stationMovingTo || "", placeholder: "Next station" },
      { label: "Move Time", name: "stationMovingTime", value: data.stationMovingTime || "", placeholder: "HH:MM" }
    ],
    (values) => {
      data.stationAssignment = values.stationAssignment.trim();
      data.stationMoving = values.stationMoving === "on";
      data.stationMovingTo = data.stationMoving ? values.stationMovingTo.trim() : "";
      data.stationMovingTime = data.stationMoving ? values.stationMovingTime.trim() : "";
      saveData();
      render();
    }
  );
...
function editBreakTimes() {
  ...
  openDialog(
    ...
    [
      ...
      { label: "Break 3", name: "break3", value: breaks[2].time, placeholder: "HH:MM", maxLength: 5, inputMode: "numeric", pattern: "[0-9]{2}:[0-9]{2}", className: "military-time-input" },
      { label: "Break 3 Taken", name: "break3Taken", type: "checkbox", checked: breaks[2].taken }
    ],
    (values) => {
      const break1 = completeMilitaryTime(values.break1);
      const break2 = completeMilitaryTime(values.break2);
      const break3 = completeMilitaryTime(values.break3);
      data.breakTimes = [
        { time: break1, taken: values.break1Taken === "on" },
        { time: break2, taken: values.break2Taken === "on" },
        { time: break3, taken: values.break3Taken === "on" }
      ];
      saveData();
      render();
    }
...
function startBreak(index) {
  ...
  els.breakStation.textContent = data.stationAssignment && data.stationAssignment !== "Set Station" ? data.stationAssignment : "Station Not Set";
...
function copyCadShortcut(key, buttonElement) {
  copyText(CAD_COPY_SHORTCUTS[key]);
...
function addChannelUse() {
  ...
  const dispatcherAssigned = els.dispatcherAssignedButton.getAttribute("aria-pressed") === "true";

  if (!channel || (callId && !/^\d{1,3}$/.test(callId))) {
    els.channelUseInput.reportValidity();
    els.callIdInput.reportValidity();
    return;
  }

  data.channelUses.unshift({ id: crypto.randomUUID(), channel, callId, dispatcherAssigned });
  els.channelUseForm.reset();
  setDispatcherAssigned(false);
...
function setDispatcherAssigned(isAssigned) {
  els.dispatcherAssignedButton.setAttribute("aria-pressed", String(isAssigned));
  els.dispatcherAssignedButton.classList.toggle("is-active", isAssigned);
}
...
function clearWeather() {
  els.weatherAddressInput.value = "";
  els.weatherAddressChoice.hidden = true;
  els.weatherAddressChoice.innerHTML = "";
  els.weatherResult.className = "weather-result";
  els.weatherResult.textContent = "Enter an address for current observed weather.";
  els.weatherAddressInput.focus();
}
...
els.copy911AButton.addEventListener("click", () => copyCadShortcut("911A", els.copy911AButton));
els.copy911UButton.addEventListener("click", () => copyCadShortcut("911U", els.copy911UButton));
els.copy911UPocketButton.addEventListener("click", () => copyCadShortcut("911U Pocket", els.copy911UPocketButton));
els.copy911OButton.addEventListener("click", () => copyCadShortcut("911O", els.copy911OButton));
els.copyPatientDeterioratingButton.addEventListener("click", () => copyCadShortcut("Patient Deteriorating", els.copyPatientDeterioratingButton));
...
els.weatherClearButton.addEventListener("click", clearWeather);
...
els.dispatcherAssignedButton.addEventListener("click", () => {
  setDispatcherAssigned(els.dispatcherAssignedButton.getAttribute("aria-pressed") !== "true");
});
