...
const els = {
  startupHud: document.querySelector(".startup-hud"),
  startupHudTitle: document.querySelector("#startupHudTitle"),
  startupHudStatus: document.querySelector("#startupHudStatus"),
...
function prepareData(nextData) {
  ...
  if (!["animated", "still"].includes(nextData.visualMode)) nextData.visualMode = "animated";
...
function applyVisualMode() {
  const isStill = data.visualMode === "still";
  document.body.classList.toggle("is-still-mode", isStill);
  if (els.visualModeButton) {
    els.visualModeButton.setAttribute("aria-pressed", String(!isStill));
    els.visualModeButton.title = isStill ? "Still mode on. Click for animated mode." : "Animated mode on. Click for still mode.";
  }
}

function toggleVisualMode() {
  data.visualMode = data.visualMode === "still" ? "animated" : "still";
  saveData();
  applyVisualMode();
}
...
els.visualModeButton?.addEventListener("click", toggleVisualMode);
...
function runStartupHud() {
  if (!els.startupHudStatus || !els.startupHudTitle) return;
  STARTUP_MESSAGES.forEach((message, index) => {
    window.setTimeout(() => {
      els.startupHudStatus.textContent = message;
      if (index === STARTUP_MESSAGES.length - 1) {
        els.startupHudTitle.textContent = "J.A.R.V.I.S. INITIALIZING";
      }
    }, 520 + index * 620);
  });
  window.setTimeout(() => {
    els.startupHud?.classList.add("is-complete");
  }, 5200);
}
